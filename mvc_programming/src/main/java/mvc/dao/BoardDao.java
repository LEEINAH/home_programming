...
public class BoardDao {
	...
	public int boardInsert(BoardVo bv) {
		...
		String sql3 = "SELECT MAX(bidx) AS maxbidx FROM board";
		try {
			...
			pstmt.executeUpdate(); // 실행 되면 1 안되면 0
	...
	public int boardUpdate(BoardVo bv) {
		...
		String sql = "UPDATE BOARD\r\n"
				+ "SET subject=?, contents=?, writer=?, modifyDay=now(), fileName=?\r\n"
		...
		try {
			...
			pstmt.setString(6, bv.getPassword());
